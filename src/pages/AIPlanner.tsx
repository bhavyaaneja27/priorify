            ...
            {(['list', 'calendar', 'table'] as const).map(v => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-all ${
                  view === v ? 'bg-accent-blue/10 text-accent-blue' : 'text-dark-400 hover:text-dark-300'
                }`}
              >
                {v}
              </button>
                        ...
                        {plan.schedule.map((day: any, idx: number) => (
                          <div
                            ...
                            className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${
                              day.completed ? 'bg-accent-green/5 border-accent-green/20' : 'bg-dark-950/60 border-dark-600/30'
                            }`}
                          >
                            <button
